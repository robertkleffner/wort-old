module.exports = {
    ...
    lex: function(input) {
        ...
        while (!this.eof()) {
            ...
            if ((this.sequence('->') || this.sequence('<-')) && this.validIdStart(this.peek(2))) {
                tokens.push(this.lexObjectModifier(input));
                continue;
            }

            // object property read syntax
            if (this.currentIs('@') && this.validIdStart(this.peek())) {
                tokens.push(this.lexObjectRead(input));
                continue;
            }

            // string literals
            if (this.currentIs('"')) {
                tokens.push(this.lexString(input));
                continue;
            }
    ...
    validIdentifier: function(char) {
        // TODO: allow unicode characters that aren't reserved symbols
        return /[A-Za-z0-9]/.test(char) || char == '.' || char == '_';
    },
