...
module.exports = {
    ...
    emitMain: function() {
        return '})();' + (this.mod.hasMain ? this.node(0, 0, '$1.main([]);') : '');
    },
    ...
    emitTerms: function(terms, inQuot, line, column) {
        ...
        var output = this.node(terms[0].line, terms[0].column, '');
        for (var i = 0; i < terms.length; i++) {
            if (terms[i] instanceof Array) {
                if (inQuot) {
                    output.add(['[', this.emitTerms(terms[i], true), ']'])
                    if (i != terms.length - 1) {
                        output.add(',');
                    }
                } else {
                    output.add(['stack.push([', this.emitTerms(terms[i], true), ']);']);
                }
            } else {
                output.add(this.emits[terms[i].type].call(this, terms[i], inQuot));
    ...
    emitShuffleFunc: function(shuffle) {
        var halves = shuffle.split('-'), output = this.node(0, 0, '');
