...
module.exports = {
    ...
    emitRequires: function() {
        var output = 'var std = require("wort-std");\n';
    ...
    emitTerms: function(terms, inQuot) {
        ...
        for (var i = 0; i < terms.length; i++) {
            if (terms[i] instanceof Array) {
                output += 'stack[++std.ind] = [' + this.emitTerms(terms[i], true) + ']' + (inQuot ? ',' : ';\n');
            } else {
                output += this.emits[terms[i].type].call(this, terms[i], inQuot) + (inQuot ? ',' : ';\n');
            }
        }
        return output;
    },
    ...
    emitLiteral: function(lit, inQuot) {
        if (!inQuot) {
            return 'stack[++std.ind] = ' + lit.value;
        }
        return lit.value;
    },
